...
CarnivoreStates_FindWater::CarnivoreStates_FindWater()
{
}
...
void CarnivoreStates_FindWater::Enter(Creature* pCreature)
{
	std::cout << "Test :" << "Carnivore State FindWater Destination\n";
...
void CarnivoreStates_FindWater::Execute(Creature* pCreature)
{
	if (pCreature == nullptr) return;
	...
	std::cout << "Carnivre finding water destination : " << creature->destination->location.x << " , " << creature->destination->location.y <<", Thirsty "<<creature->getThirst() <<std::endl;
...
void CarnivoreStates_FindWater::Resume(Creature* pCreature)
{
	return;
}

void CarnivoreStates_FindWater::Exit(Creature* pCreature)
{
	if (pCreature == nullptr) return;
	...
	std::cout << "Carnivore Get Water " << creature->getThirst() << std::endl;
...
bool CarnivoreStates_FindWater::canVisit(Location& testLocation)
{
	return Map[testLocation.x][testLocation.y].animal == nullptr;
}

bool CarnivoreStates_FindWater::isVisitedAndUpdate(std::priority_queue<pathInfo, std::vector<pathInfo>, cmp>& pathList_visited, std::list<pathInfo>& pathList_finished, Location& testLocation, pathInfo& fatherInfo, Creature* destination)
{
...
void CarnivoreStates_FindWater::getClosest(pathInfo& tempPath)
{
...
bool CarnivoreStates_FindWater::arrive(pathInfo& currentLocation, Carnivore* finder)
{
...
void CarnivoreStates_FindWater::findRoad(std::list<pathInfo>& pathList, Carnivore* finder)
{
...
Creature* CarnivoreStates_FindWater::getDestination(std::vector<Info*>& infoVector, Carnivore* finder)
{
	int d, mini;
...
Creature* CarnivoreStates_FindWater::getDestinationFromMemory(std::vector<MemoryItem>& infoVector, Carnivore* finder)
{
	int d, mini;
...
CarnivoreStates_FindWater* CarnivoreStates_FindWater::Instance()
{
	static CarnivoreStates_FindWater CarnivoreStates_FindWater_instance;
	return &CarnivoreStates_FindWater_instance;
}


int CarnivoreStates_FindWater::getDistance(Location& l1, Location& l2)
{
...
int CarnivoreStates_FindWater::getMin(int a, int b)
{
	if (a > b) return b;
	else return a;
}
